...
def plummer(Npart, a, m=1., G=4.49e-11, seed=None):
    """
    ...
    Npart = int(Npart)
    pos = PlummerDist_2d_xy(Npart, a, rand=rand)
    if np.size(m) == 1:
    ...
    posval = np.concatenate((pos, vel), axis=1)
   
    return posval

# Spatial Distribution for Plummer Model
def PlummerDist_2d_xy(Npart, a, rand=np.random):
    """
    ...
    for i in range(Npart):
        ...
        pos[i] = r[i]*rand_unit_vector(2, rand=rand)
    return pos+np.array([250,250])
...
def velDist_Plummer(Npart, r, M, a, G, rand=np.random):
    ...
    for i in range(Npart):
        ...
        vel[i, :] = rand_unit_vector(2, rand=rand)*vel_mod
    return vel*100
...
def circVel_Plummer(r, M, a, G, rand=np.random):
    ...
    vel = np.zeros((N, 2))
    for i in range(N):
        v_hat = rand_unit_vector(2, rand=rand)
...
def main():
    ...
    bodies = plummer(1000, 1, m=1e8, seed=10)
